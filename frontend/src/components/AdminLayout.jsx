...
export default function AdminLayout() {
  ...
  return (
    <div className="min-h-screen flex" style={{ 
      background: 'radial-gradient(circle at 20% 20%, rgba(255, 107, 0, 0.12), transparent), #0B0B0D'
    }}>
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 flex flex-col transform transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0`}
        style={{ background: 'linear-gradient(180deg, #0B0B0D 0%, #141416 100%)', borderRight: '1px solid rgba(255, 107, 0, 0.1)' }}>
        <div className="h-16 flex items-center justify-between px-5" style={{ borderBottom: '1px solid rgba(255, 107, 0, 0.1)' }}>
        ...
        <div className="p-4" style={{ borderTop: '1px solid rgba(255, 107, 0, 0.1)' }}>
          <div className="flex items-center gap-3 p-3 rounded-lg" style={{ background: 'rgba(255, 107, 0, 0.08)', border: '1px solid rgba(255, 107, 0, 0.15)' }}>
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#FF6B00] to-[#E55A00] flex items-center justify-center text-white text-xs font-bold shadow-lg shadow-orange-500/30">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="h-16 flex items-center justify-between px-4 sm:px-6 z-10 sticky top-0"
          style={{ background: 'rgba(11, 11, 13, 0.9)', backdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(255, 107, 0, 0.1)' }}>
          ...
          <div className="flex items-center gap-3 ml-auto">
            ...
            <div className="relative">
              <button onClick={() => setNotifOpen(!notifOpen)} className="relative p-2 rounded-lg text-gray-400 hover:text-white hover:bg-white/5 transition">
                ...
                <span className="absolute top-1.5 right-1.5 block h-2 w-2 rounded-full bg-[#FF6B00] shadow-lg shadow-orange-500/50"></span>
              </button>
              {notifOpen && (
                <>
                  <div className="fixed inset-0 z-10" onClick={() => setNotifOpen(false)} />
                  <div className="absolute right-0 mt-2 w-72 rounded-xl overflow-hidden z-20 shadow-2xl" style={{ background: '#141416', border: '1px solid rgba(255, 107, 0, 0.2)' }}>
                    <div className="p-3 flex justify-between items-center" style={{ borderBottom: '1px solid rgba(255, 107, 0, 0.1)' }}>
                      <h3 className="text-sm font-bold text-white">Notifications</h3>
                      <span className="text-xs text-[#FF6B00] cursor-pointer hover:text-[#E55A00] transition">Mark all read</span>
                    </div>
            ...
            {/* Profile */}
            <div className="relative" style={{ borderLeft: '1px solid rgba(255, 107, 0, 0.1)', paddingLeft: '12px' }}>
              <button onClick={() => setDropdownOpen(!dropdownOpen)} className="flex items-center gap-2 p-1.5 rounded-lg hover:bg-white/5 transition">
                <div className="w-7 h-7 rounded-full bg-gradient-to-br from-[#FF6B00] to-[#E55A00] flex items-center justify-center text-white text-xs font-bold shadow-lg shadow-orange-500/30">
                  {user?.first_name?.[0]}{user?.last_name?.[0]}
                </div>
              ...
              {dropdownOpen && (
                <>
                  ...
                  <div className="absolute right-0 mt-2 w-44 rounded-xl overflow-hidden z-20 shadow-2xl" style={{ background: '#141416', border: '1px solid rgba(255, 107, 0, 0.2)' }}>
                    <div className="p-1.5">
